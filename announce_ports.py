...
def post_announcement(content, embed_data):
    """
    Posts a message with a rich embed to Discord using the bot API,
    then crossposts (publishes) it to announcement channel followers.
    """
    ...
    try:
        # Step 1: Send the message to the announcement channel
        send_url = f"{DISCORD_API_BASE}/channels/{CHANNEL_ID}/messages"
        ...
        # Step 2: Crosspost (publish) the message
        if message_id:
            crosspost_url = f"{DISCORD_API_BASE}/channels/{CHANNEL_ID}/messages/{message_id}/crosspost"
            crosspost_response = requests.post(crosspost_url, headers=headers, timeout=10)
            crosspost_response.raise_for_status()
            print(f"Successfully crossposted message ID: {message_id}")
        else:
            print("Warning: Could not get message ID for crossposting")
